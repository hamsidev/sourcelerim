...
void CGuildManager::ParseResult(SQLResult * pRes)
{
	...
	while ((row = mysql_fetch_row(pRes->pSQLResult)))
	{
		...
		str_to_number(r_info.ronark, row[8]);
#endif
...
{
	char szQuery[1024];
	snprintf(szQuery, sizeof(szQuery), "SELECT id, name, ladder_point, win, draw, loss, gold,"
#ifdef ENABLE_RONARK_SYSTEM
						" ronark, "
#endif
					"level FROM guild%s "
...
void CGuildManager::Load(DWORD dwGuildID)
{
	...
	snprintf(szQuery, sizeof(szQuery), "SELECT id, name, ladder_point, win, draw, loss, gold,"
						...
						" ronark, "
#endif
						"level FROM guild%s WHERE id=%u"
						,
						GetTablePostfix(), dwGuildID);
...
#ifdef ENABLE_RONARK_SYSTEM
void CGuildManager::RonarkKing(DWORD dwGuild, bool ronark_king)
{
	...
	it->second.ronark = ronark_king;

	char buf[1024];
	snprintf(buf, sizeof(buf), "UPDATE guild%s SET ronark=%d WHERE id = %u", GetTablePostfix(), ronark_king, dwGuild);
	CDBManager::instance().AsyncQuery(buf);

	TPacketDGGuildRonarkKing p;
	p.dwGuild = dwGuild;
	p.ronark_king = ronark_king;
	CClientManager::instance().ForwardPacket(HEADER_DG_GUILD_RONARK_KING, &p, sizeof(p));

	sys_log(0, "GUILD: %u Ronark King %d to %d", dwGuild, ronark_king, it->second.ronark);
}
#endif
