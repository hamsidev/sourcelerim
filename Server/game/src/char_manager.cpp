...
#endif
{
	...
	if (ch->IsNPC() && !ch->IsPet() && !ch->IsMount() && ch->GetRider() == nullptr)
#else
...
struct FuncUpdateAndResetChatCounter
{
	void operator () (LPCHARACTER ch)
	{
		...
		ch->CFSM::Update();
#ifdef ENABLE_RONARK_SYSTEM
		CRonarkMapManager::instance().SendScorePacket();
#endif
	}
};
...
#ifdef on
int64_t	CHARACTER_MANAGER::GetUserDamageRate(LPCHARACTER ch)
#else
