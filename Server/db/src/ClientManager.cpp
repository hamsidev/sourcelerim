			...
			if (!bEscape)
			{
				...
				while (it != pSet->end())
				{
...
#ifdef ENABLE_EXTRA_SOCKET_SYSTEM
						dwSocket3 = (lValue2 == 0) ? static_cast<DWORD>(lValue0) : static_cast<DWORD>(tNow + lValue0);
#endif
						#else
						if (pItemAward->dwSocket2 != 0)
							dwSocket2 = pItemAward->dwSocket2;
#ifdef ENABLE_EXTRA_SOCKET_SYSTEM
						else if (pItemAward->dwSocket3 != 0)
							dwSocket3 = pItemAward->dwSocket3;
#endif
						else
...

					{
						...
						iValueLen += snprintf(szValues + iValueLen, sizeof(szValues) - iValueLen, ", %u, %u, %u, %u", pItemAward->dwSocket0, pItemAward->dwSocket1, dwSocket2, dwSocket3);
#else
...
void CClientManager::GuildRonarkKing(TPacketGDRonarkKing* p)
{
	CGuildManager::instance().RonarkKing(p->dwGuild, p->ronark_king);
}
#endif
	...
	while (peer->PeekPacket(i, header, dwHandle, dwLength, &data))
	{
		...
		switch (header)
		{
			...
			case HEADER_GD_REQUEST_CHANNELSTATUS:
				...
				break;
#ifdef ENABLE_RONARK_SYSTEM
		case HEADER_GD_GUILD_RONARK_KING:
			GuildRonarkKing((TPacketGDRonarkKing*)data);
			break;
#endif
