...
using namespace std;
#include <sstream>
#define SOCKET_START_IDX 5	//on mysql table.
#define MAX_SOCKET 4	//on mysql table.
#define MAX_ATTRIBUTE 7
#define ATTRIBUTE_START_IDX (SOCKET_START_IDX + MAX_SOCKET)
#define GET_TO_INVENTORY
#define SOCKETIDX_ROW_IDX 23
#define TIME_TYPE_ROW_IDX 24
#define COUNT_ROW_IDX 25
...
void CInputMain::ItemShop (LPCHARACTER ch, const char* c_pData)
{
	...
	switch (p->bSubHeader)
	{
		case ITEMSHOP_PROCESS_REFRESH:
		{
			...
			if ((string)p->szCategoryStr != "_GirisYenile_")
			{
				goto ItemQuery;
			}
			{
...
ItemQuery:
			;
			{
				...
				if (ch->IsGM())
				{
					ch->ChatPacket (1, "qRy: [%s]", szQuery2);
				}
				...
				else
				{
					...
					while (NULL != (mRow = mysql_fetch_row (pMsg_Item->Get()->pSQLResult)))
					{
						...
						{
							str_to_number (isItem.alSockets[i], mRow[iCur++]);
						}
		...
		case ITEMSHOP_PROCESS_BUY:
		{
			...
			if (!p->itemID)
			{
				sys_err ("abnormal!!! ITEMSHOP_PROCESS_BUY > [%s] p->itemID was null, returned.", ch->GetName());
				return;
			}
			if (!p->itemCount)
			{
				sys_err ("abnormal!!! ITEMSHOP_PROCESS_BUY > [%s] p->itemCount was null, returned.", ch->GetName());
				return;
			}
			if (ch->GetDesc()->GetAccountTable().id == 0)
			{
				sys_err ("abnormal!!! ITEMSHOP_PROCESS_BUY > [%s] account ID = 0.", ch->GetName());
				return;
			}
			...
			const bool isVC = (priceType == 1);

			DWORD myCoins = isVC ? ch->GetVoteCoins() : ch->GetCoins();

			if (myCoins < itemPrice * p->itemCount)
			{
				ch->ChatPacket (1, "Yeterli %s %d > %d.", (isVC ? "Vote Coins" : "EP"), itemPrice, myCoins);
			...
			if (hasTime)
			{
				...
				const bool isRealTime = (timeType == LIMIT_REAL_TIME || timeType == LIMIT_REAL_TIME_START_FIRST_USE);
				if (iSocketIdx >= MAX_SOCKET)
				{
					goto End;
				}
...
End:
			...
			for (BYTE i = 0; i < p->itemCount; i++)
			{
				...
				{
					windowType = UPGRADE_INVENTORY;
					invPos = ch->GetEmptySpecialStorageSlot (item);
				}

				else if (item->IsBook())
				{
					windowType = BOOK_INVENTORY;
					invPos = ch->GetEmptySpecialStorageSlot (item);
				}

				else if (item->IsStone())
				{
					windowType = STONE_INVENTORY;
					invPos = ch->GetEmptySpecialStorageSlot (item);
				}

				else if (item->IsGeneralItem())
				{
					windowType = GENERAL_INVENTORY;
					invPos = ch->GetEmptySpecialStorageSlot (item);
				}

				else if (item->IsBonus())
				{
					windowType = BONUS_INVENTORY;
					invPos = ch->GetEmptySpecialStorageSlot (item);
				}
				...
				else
				{
					...
					invPos = ch->GetEmptyInventory (pTable->bSize);
				}
				...
				item->AddToCharacter (ch, TItemPos (windowType, invPos));
				std::unique_ptr<SQLMsg> sqlLog (
					DBManager::instance().DirectQuery ("INSERT INTO log.itemshop_log(item_vnum, player_pid, player_name, item_price, buy_date) VALUES(%d, %d, '%s', %d, NOW())", itemVnum,
													   ch->GetPlayerID(), ch->GetName(), itemPrice));
	...
	switch (pinfo->sub_header)
	{
		...
		case EXCHANGE_SUBHEADER_CG_ELK_ADD:	// arg1 == amount of gold
			if (ch->GetExchange())
			{
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("»ó´ëacRÇ AN±Yl×RE 20ld lÉR» AE°úÇDz© °L·^¸¦ ÇNLö°^ lr¨R´D´U.."));
...
#ifdef ENABLE_SPECIAL_STORAGE
	if (!pkItem->IsDragonSoul() && !pkItem->IsSpecialStorageItem())
#else
...
void CInputMain::SafeboxCheckout(LPCHARACTER ch, const char * c_pData, bool bMall)
{
	...
	else if (pkItem->IsSpecialStorageItem())
	{
		...
		if (iCell < 0)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("|T<Storages> No movement possible.|t"));
			return ;
		}
		// DestPos = TItemPos (INVENTORY, p->ItemPos);
	...
	else
	{
		if (DRAGON_SOUL_INVENTORY == p->ItemPos.window_type
#ifdef ENABLE_SPECIAL_STORAGE
			|| pkItem->GetSpecialWindowType() == p->ItemPos.window_type
#endif
		)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Ã¢°í> ¿Å±æ ¼ö ¾ø´Â À§Ä¡ÀÔ´Ï´Ù."));
...
void CInputMain::PartyInvite(LPCHARACTER ch, const char * c_pData)
{
	...
	if (!pInvitee || !ch->GetDesc() || !pInvitee->GetDesc() || !pInvitee->IsPC() || !ch->IsPC()) //@hinata30
	{
...
void CInputMain::PartyInviteAnswer(LPCHARACTER ch, const char * c_pData)
{
	...
	if (!pInviter || !pInviter->IsPC()) //@hinata30
...
#ifdef ENABLE_PARTY_DUNGEON_FIX
			int pDungeonIndex[] = { /*SPIDER_DUNGEON_BOSS_INDEX,*/
									/**KIPIA_DUNGEON_BOSS_INDEX,*/
									DEVIL_CATACOMB_INDEX,
									DEVIL_TOWER_INDEX,
									FLAME_DUNGEON_INDEX,
									SNOW_DUNGEON_INDEX };

			for (int index = 0; index < _countof(pDungeonIndex); index++)
			{
				if (pParty->IsPartyInDungeon(pDungeonIndex[index]))
				{
		...
		case GUILD_SUBHEADER_CG_REMOVE_MEMBER:
			{
				...
				if (member)
				{
					...
					{
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Lonca savasi icerisindeyken uyeyi kaldiramazsiniz."));
						return SubPacketLen;
					}
#endif
...
void CInputMain::InstantChestOpen(LPCHARACTER ch, const char* c_pData)
{
	...
	auto * pInfo	= (TInstantChestOpen*)c_pData;
	ch->UseItemAll(pInfo->iPos);
}
#endif
...
void CInputMain::ChestDropInfo(LPCHARACTER ch, const char* c_pData)
{
	...
	if (p->wInventoryCell >= SPECIAL_INVENTORY_MAX_NUM)
		return;
	LPITEM pkItem = ch->GetGeneralInventoryItem(p->wInventoryCell);
	if (!pkItem)
		return;
...
#ifdef ITEM_SHOP
		case HEADER_CG_ITEMSHOP_PROCESS:
		{
			ItemShop (ch, c_pData);
			break;
		}
