...
bool ITEM_MANAGER::ReadWhellRewards(const char* c_pszFileName)
{
	for (auto it = m_map_pkWhellItemGroup.begin(); it != m_map_pkWhellItemGroup.end(); ++it)
		delete it->second;
	m_map_pkWhellItemGroup.clear();
	...
	for (DWORD i = 0; i < loader.GetChildNodeCount(); ++i)
	{
		...
		loader.GetCurrentNodeName(&stName);
		loader.SetChildNode(i);
		if (!loader.GetTokenInteger("type", &iWhellType))
		{
			sys_err("ReadWhellRewards : Syntax error %s : no type, node %s", c_pszFileName, stName.c_str());
			loader.SetParentNode();
			return false;
		}
		CWhellItemGroup* pkGroup = M2_NEW CWhellItemGroup(iWhellType);
		for (int k = 1; k < 256; ++k)
		{
			char buf[4];
			snprintf(buf, sizeof(buf), "%d", k);
			...
			{
				DWORD dwVnum = 0;
				str_to_number(dwVnum, pTok->at(0).c_str());
				int iCount = 0;
				str_to_number(iCount, pTok->at(1).c_str());
				if (iCount < 1)
				{
					sys_err("ReadWhellRewards : there is no count for item %u : node %s : vnum %d, count %d", dwVnum, stName.c_str(), dwVnum, iCount);
					M2_DELETE(pkGroup);
					return false;
				}
				int iRarePct = 0;
				str_to_number(iRarePct, pTok->at(2).c_str());
				if (iRarePct == 0)
				{
					sys_err("ReadWhellRewards : there is no drop percent for item %u : node %s : vnum %d, count %d, pct %d", dwVnum, stName.c_str(), iRarePct);
					M2_DELETE(pkGroup);
					return false;
				}
				pkGroup->AddItem(dwVnum, iCount, iRarePct, MINMAX(0, iRarePct, 100));
				continue;
			}
			break;
		}
		// sys_err("whell: %u size: %u", iWhellType, pkGroup->GetVector()->size());
		if (pkGroup->IsEmpty())
		{
			M2_DELETE(pkGroup);
			continue;
		}
		m_map_pkWhellItemGroup.insert(std::map<DWORD, CWhellItemGroup*>::value_type(iWhellType, pkGroup));
		loader.SetParentNode();
	}
	return true;
}
