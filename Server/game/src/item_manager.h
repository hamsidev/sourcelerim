...
{
public:
	struct SWhellItemGroupInfo
	{
		DWORD dwItemVnum;
		int iCount, iRarePct;
		SWhellItemGroupInfo(DWORD dwItemVnum, int iCount, int iRarePct): dwItemVnum(dwItemVnum),iCount(iCount),iRarePct(iRarePct){}
	};
	CWhellItemGroup(DWORD dwMobVnum):m_dwMobVnum(dwMobVnum){}
	...
	void AddItem(DWORD dwItemVnum, int iCount, int iPartPct, int iRarePct)
	{
		...
		m_vecItems.emplace_back(SWhellItemGroupInfo(dwItemVnum, iCount, iRarePct));
	}
	int GetOneIndex() const
	{
		int n = number(1, m_vecProbs.back());
	...
	const SWhellItemGroupInfo& GetOne() const
	{
		return m_vecItems[GetOneIndex()];
	}
	std::vector<SWhellItemGroupInfo>* GetVector() {return &m_vecItems;}
private:
	DWORD m_dwMobVnum;
	std::vector<int> m_vecProbs;
	std::vector<SWhellItemGroupInfo> m_vecItems;
};
#endif
class CMobItemGroup
{
...
#ifdef ENABLE_WHEEL_OF_FORTUNE
		bool			ReadWhellRewards(const char* c_pszFileName);
		std::map<DWORD, CWhellItemGroup*>* GetWhellItemsData() {return &m_map_pkWhellItemGroup;};
#endif
...
#ifdef ENABLE_WHEEL_OF_FORTUNE
		std::map<DWORD, CWhellItemGroup*> m_map_pkWhellItemGroup;
#endif
