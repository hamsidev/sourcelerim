...
void CPythonNetworkStream::GamePhase()
{
...
#ifdef PACKET_GONDER_KNK
	const DWORD MAX_RECV_COUNT = 0; // @warme669 DEFAULT 8
    ...
    while (ret)
	{
...
#endif
#ifdef ITEM_SHOP
			case HEADER_GC_ITEMSHOP_CATEGORY:
				ret = RecvItemShopCategoryPacket();
				break;
			case HEADER_GC_ITEMSHOP_ITEM:
				ret = RecvItemShopItemsPacket();
				break;
#endif
		...
		case HEADER_GC_PVP_RANKING:
			ret = RecvPvPRankingPacket();
			break;
#endif
...
#ifdef ITEM_SHOP
bool CPythonNetworkStream::RecvItemShopCategoryPacket()
{
	TCGItemShopSize gPacket;
	if (!Recv (sizeof (gPacket), &gPacket))
	{
		return false;
	}
	gPacket.dataSize -= sizeof (gPacket);
	WORD vIndex = 0;
	while (gPacket.dataSize > 0)
	{
		TGCIShopCategory catData;
		if (!Recv (sizeof (catData), &catData))
		{
			return false;
		}
		gPacket.dataSize -= sizeof (catData);
		vIndex++;
		if (catData.mainID == 255)
		{
			PyCallClassMemberFunc (m_apoPhaseWnd[PHASE_WINDOW_GAME], "AddCategory", Py_BuildValue ("(si)", catData.categoryName, catData.catID));
		}
		else
		{
			PyCallClassMemberFunc (m_apoPhaseWnd[PHASE_WINDOW_GAME], "FillCategory", Py_BuildValue ("(isi)", catData.mainID, catData.categoryName, catData.catID));
		}
	}

	return true;
}
bool CPythonNetworkStream::RecvItemShopItemsPacket()
{
	TCGItemShopSize gPacket;
	if (!Recv (sizeof (gPacket), &gPacket))
	{
	...
	gPacket.dataSize -= sizeof (gPacket);
	WORD vIndex = 0;
	PyCallClassMemberFunc (m_apoPhaseWnd[PHASE_WINDOW_GAME], "ClearList", Py_BuildValue ("()"));
	while (gPacket.dataSize > 0)
	{
		TGCIShopItem itemData;
		if (!Recv (sizeof (itemData), &itemData))
		{
			return false;
		}
		gPacket.dataSize -= sizeof (itemData);
		vIndex++;
		char szMetinStr[48], szAttrStr[48];
		snprintf (szMetinStr, sizeof (szMetinStr), "%d|%d|%d|%d", itemData.alSockets[0], itemData.alSockets[1], itemData.alSockets[2], itemData.alSockets[3]);
		snprintf (szAttrStr, sizeof (szAttrStr), "%d|%d`%d|%d`%d|%d`%d|%d`%d|%d`%d|%d`%d|%d", itemData.aAttr[0].bType, itemData.aAttr[0].sValue,
				  itemData.aAttr[1].bType, itemData.aAttr[1].sValue, itemData.aAttr[2].bType, itemData.aAttr[2].sValue, itemData.aAttr[3].bType, itemData.aAttr[3].sValue,
				  itemData.aAttr[4].bType, itemData.aAttr[4].sValue, itemData.aAttr[5].bType, itemData.aAttr[5].sValue, itemData.aAttr[6].bType, itemData.aAttr[6].sValue);
		PyCallClassMemberFunc (m_apoPhaseWnd[PHASE_WINDOW_GAME], "AddItem", Py_BuildValue ("(iiiiissi)", itemData.iID, itemData.iVnum, itemData.iCount, itemData.iPrice,
																						   itemData.categoryID, szMetinStr, szAttrStr, itemData.isVoteCoins ? 1 : 0));
	}

	return true;
}

bool CPythonNetworkStream::SendItemShopPacket (BYTE subHeader, BYTE itemCount, DWORD itemID, char* szCategoryStr, bool withQueryStr)
{
	TCGItemShopProcess gPacket;
	gPacket.bHeader = HEADER_CG_ITEMSHOP_PROCESS;
	gPacket.bSubHeader = subHeader;
	gPacket.itemCount = itemCount;
	gPacket.itemID = itemID;
	snprintf (gPacket.szCategoryStr, sizeof (gPacket.szCategoryStr), szCategoryStr);
	// char szDbg[128];
	// snprintf(szDbg, sizeof(szDbg), "subHeader: %d, itemCount: %d, itemId: %d, categoryStr: %s", subHeader, itemCount, itemID, gPacket.szCategoryStr);
	gPacket.withQuery = withQueryStr;
	// CPythonChat::Instance().AppendChat(1, szDbg);
	if (!Send (sizeof (gPacket), &gPacket))
	{
		return false;
	}
	return true;
	...
	switch (exchange_packet.subheader)
	{
		...
		case EXCHANGE_SUBHEADER_GC_ITEM_ADD:
			if (exchange_packet.is_me)
			{
				...
				, (DWORD)exchange_packet.arg3
#else
				, (BYTE) exchange_packet.arg3
#endif
				);
		...
		case EXCHANGE_SUBHEADER_GC_ITEM_DEL:
			...
			{
#ifdef ENABLE_ITEM_COUNT_LIMIT_SYSTEM
				CPythonExchange::Instance().DelItemOfSelf((DWORD)exchange_packet.arg1);
#else
			...
			{
#ifdef ENABLE_ITEM_COUNT_LIMIT_SYSTEM
				CPythonExchange::Instance().DelItemOfTarget((DWORD)exchange_packet.arg1);
#else
...
#ifdef ENABLE_MESSENGER_REMOVE_FIX
...
#ifdef SANDIK_AC
bool CPythonNetworkStream::SendInstantChestOpenPacket(TItemPos iPos){
	TInstantChestOpen gPacket;
	gPacket.bHeader = HEADER_CG_INSTANT_OPEN;
	gPacket.iPos = iPos;
	if (!Send(sizeof(gPacket), &gPacket)){ return false; }
	return true;
}
#endif
...
#ifdef ENABLE_RONARK_SYSTEM
bool CPythonNetworkStream::RecvRonarkInfo()
{
	TPacketGCRonark	infoPacket;
	if (!Recv(sizeof(infoPacket), &infoPacket))
		return false;

	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_RonarkUpdateInfo", Py_BuildValue("iiiii", infoPacket.iList, infoPacket.dwGuildID, infoPacket.dwDamage, infoPacket.dwMemberCount, infoPacket.dwLimit));
	return true;
}
#endif
...
#endif
#ifdef ENABLE_PVP_RANKING
bool CPythonNetworkStream::RecvPvPRankingPacket()
{
	TPacketGCPvPRanking rPacket;
	if (!Recv(sizeof(rPacket), &rPacket))
	{
		return false;
	}

	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "PVP_ClearPlayerInfo", Py_BuildValue("()"));

	// Bu if koþulunda, isRefresh deðiþkeni false olmalý
	bool isRefresh = false;

	if (rPacket.bStatus[0] == (0))
	{
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "PVP_CloseWindow", Py_BuildValue("()"));
		goto End;
	}

	for (BYTE i = 0; i < 10; i++)
	{
		if (rPacket.bLevels[i] == 0)
		{
			continue;
		}

#ifdef ENABLE_PVP_RANKING_WITH_EMPIRE
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "PVP_AddPlayer", Py_BuildValue("(siiiiiiiLLL)", rPacket.szNames[i], rPacket.bLevels[i], rPacket.bJobs[i],
			rPacket.bEmpire[i], rPacket.iKill[i], rPacket.iDead[i], rPacket.iMaxKill, rPacket.bIsRankEmpire, rPacket.llEmpire1, rPacket.llEmpire2, rPacket.llEmpire3));
#else
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "PVP_AddPlayer", Py_BuildValue("(siiiiii)", rPacket.szNames[i], rPacket.bLevels[i], rPacket.bJobs[i],
			rPacket.bEmpire[i], rPacket.iKill[i], rPacket.iDead[i], rPacket.iMaxKill));
#endif
		isRefresh = true;
	}
	if (isRefresh)
	{
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "PVP_Refresh", Py_BuildValue("()"));
	}
