...
PyObject* netSendItemUseSpecialToItemPacket(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonNetworkStream& rkNetStream=CPythonNetworkStream::Instance();
	rkNetStream.SendItemUseToItemPacket(TItemPos(type, sourceCell), TItemPos(INVENTORY, targetCell));
	return Py_BuildNone();
}
#endif
...
PyObject* netSendSpecialMovePacket(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendItemMovePacket(TItemPos(type, sourceCell), TItemPos(INVENTORY, targetCell), (WORD)count);
	return Py_BuildNone();
}
#endif
...
#ifdef SANDIK_AC
PyObject* netSendInstantChestOpenPacket(PyObject* poSelf, PyObject* poArgs)
{
	TItemPos iPos;
	if (!PyTuple_GetInteger(poArgs, 0, &iPos.window_type)){ return Py_BuildException(); }
	if (!PyTuple_GetInteger(poArgs, 1, &iPos.cell)){ return Py_BuildException(); }

	CPythonNetworkStream::Instance().SendInstantChestOpenPacket(iPos);
	return Py_BuildNone();
}
#endif
...
#ifdef ITEM_SHOP
PyObject* netSendItemShopPacket (PyObject* poSelf, PyObject* poArgs)
{
	BYTE subHeader, itemCount;
	DWORD itemID;
	char* szQuery;
	bool withQuery;
	...
	if (!PyTuple_GetInteger (poArgs, 1, &itemCount))
	{
		return Py_BuildException();
	}
	if (!PyTuple_GetUnsignedLong (poArgs, 2, &itemID))
	{
		return Py_BuildException();
	}
	if (!PyTuple_GetString (poArgs, 3, &szQuery))
	{
		return Py_BuildException();
	}
	if (!PyTuple_GetBoolean (poArgs, 4, &withQuery))
	{
		return Py_BuildException();
	}
	CPythonNetworkStream::Instance().SendItemShopPacket (subHeader, itemCount, itemID, szQuery, withQuery);
	return Py_BuildNone();
}
#endif
...
void initnet()
{
	static PyMethodDef s_methods[] =
	{
...
#ifdef SANDIK_AC
		{ "SendInstantChestOpenPacket",			netSendInstantChestOpenPacket,			METH_VARARGS },
#endif
		...
		{ "SendItemUseSpecialToItemPacket",			netSendItemUseSpecialToItemPacket,			METH_VARARGS },
#endif
#ifdef ENABLE_TARGET_INFORMATION_SYSTEM
...
#endif
#ifdef ITEM_SHOP
		{ "SendItemShopPacket",						netSendItemShopPacket,						METH_VARARGS },
#endif
	...
	PyModule_AddIntConstant(poModule, "ACCOUNT_CHARACTER_SLOT_WING", CPythonNetworkStream::ACCOUNT_CHARACTER_SLOT_WING);
#endif
...
#ifdef ITEM_SHOP
	PyModule_AddIntConstant (poModule, "PROCESS_ITEMSHOP_REFRESH", ITEMSHOP_PROCESS_REFRESH);
	PyModule_AddIntConstant (poModule, "PROCESS_ITEMSHOP_BUY", ITEMSHOP_PROCESS_BUY);
#endif
}
