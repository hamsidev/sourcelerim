...
enum EPointTypes
{
	...
	POINT_ATTBONUS_STONE = 164,
#endif
#ifdef ENABLE_BONUS_BOSS
	POINT_ATTBONUS_BOSS = 165,
#endif
#ifdef NEW_PVM_ATTRIBUTE
	POINT_PVM_NORM_HIT_BONUS = 166,
#endif
...
struct PlayerSlotT {
	...
	std::array<LPITEM,SPECIAL_INVENTORY_MAX_NUM> pSSUItems;
	std::array<WORD,SPECIAL_INVENTORY_MAX_NUM> wSSUItemGrid;
	std::array<LPITEM,SPECIAL_INVENTORY_MAX_NUM> pSSBItems;
	std::array<WORD,SPECIAL_INVENTORY_MAX_NUM> wSSBItemGrid;
	std::array<LPITEM,SPECIAL_INVENTORY_MAX_NUM> pSSSItems;
	std::array<WORD,SPECIAL_INVENTORY_MAX_NUM> wSSSItemGrid;
	std::array<LPITEM,SPECIAL_INVENTORY_MAX_NUM> pSSGItems;
	std::array<WORD,SPECIAL_INVENTORY_MAX_NUM> wSSGItemGrid;
	std::array<LPITEM,SPECIAL_INVENTORY_MAX_NUM> pSSBOItems;
	std::array<WORD,SPECIAL_INVENTORY_MAX_NUM> wSSBOItemGrid;
#endif
...
//		bool			IsPet() const		{ return m_bCharType == CHAR_TYPE_PET; }
	...
	int	GetRankKill()
	{
		return m_points.rank_Kill;
	}
	int	GetRankDead()
	{
		return m_points.rank_Dead;
	}
	void	SetRankKill(int count)
	{
		m_points.rank_Kill = count;
	}
	void	SetRankDead(int count)
	{
		m_points.rank_Dead = count;
	}
#endif
#ifdef on
...
, BYTE num
		...
		std::pair<bool, LPITEM>	AutoStackItemProto(DWORD dwItemVnum
#ifdef ENABLE_ITEM_COUNT_LIMIT_SYSTEM
		, DWORD & bCount
#else
		, BYTE & bCount
	...
	public:
		...
		int				GetEmptySpecialStorageSlot(LPITEM pItem) const;
		int				GetEmptySpecialStorageSlotFromIndex(WORD index, BYTE itemSize, int invType) const;
		DWORD			GetSpecialWindowType(DWORD eType) const;
		...
		LPITEM			GetGeneralInventoryItem(WORD wCell) const;
		LPITEM			GetBonusInventoryItem(WORD wCell) const;

#endif

#ifdef SANDIK_AC
		int				GetEmptyAll(BYTE bSize, BYTE bWindow) const;
		int				GetEmptyAll(LPITEM pItem) const;
#endif
...
protected:
	...
	public:
		...
		void			OpenMyShop(const char * c_pszSign, TShopItemTable * pTable
#ifdef ENABLE_ITEM_COUNT_LIMIT_SYSTEM
		, DWORD bItemCount
#else
		, BYTE bItemCount
 ...
 //mt2dev cannot tree fixi
...
#ifdef ENABLE_FAST_EQUIP_FIX
		void		SetLastItemFastWear(int time) { m_dwItemFastWearLast = time; }
		int			GetLastItemFastWear() const { return m_dwItemFastWearLast; }
#endif
	...
	public:
		void	OnAlignUpdate(BYTE now_align_index);
		int		GetAlignIndex(int m_iRealAlignment);
#endif
