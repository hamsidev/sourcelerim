...
bool CHARACTER::LearnSkillByBook(DWORD dwSkillVnum, BYTE bProb)
{
	...
	{
		if (GetLevel() < PLAYER_MAX_LEVEL_CONST) { need_exp = 20000; }//@hinata13
...
struct FuncSplashDamage
{
	...
	void operator () (LPENTITY ent)
	{
...
#ifdef on
			iAmount = (int64_t) (iAmount * adjust);
#else
		...
		{
#ifdef on
			iDam = (int64_t) (iDam * m_pkSk->kSplashAroundDamageAdjustPoly.Eval());
#else
...
bool CHARACTER::UseSkill(DWORD dwVnum, LPCHARACTER pkVictim, bool bUseGrandMaster)
{
	...
	if ((dwVnum == SKILL_GEOMKYUNG || dwVnum == SKILL_GWIGEOM) && !GetWear(WEAR_WEAPON))//@hinata9
		return false;
