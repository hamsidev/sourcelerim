...
#endif
#ifdef ENABLE_PVP_RANKING
	event_cancel (&m_pkPvPRankingEvent);
#endif
...
#endif
#ifdef ENABLE_WINGS_NEW
	pack.awPart[CHR_EQUIPPART_WING] = GetPart(PART_WING);
#endif
	pack.bMovingSpeed	= GetLimitPoint(POINT_MOV_SPEED);
	pack.bAttackSpeed	= GetLimitPoint(POINT_ATT_SPEED);
	pack.bStateFlag	= m_bAddChrState;
...
#ifdef ENABLE_PVP_RANKING
	tab.rank_Kill = m_points.rank_Kill;
	tab.rank_Dead = m_points.rank_Dead;
#endif
...
void CHARACTER::Disconnect(const char * c_pszReason)
{
	...
	if (GetProtectTime("WheelWorking") != 0)
	{
		const DWORD dwSelectedItemIdx = GetProtectTime("WheelLuckyItemVnum"), dwSelectedItemCount = GetProtectTime("WheelLuckyItemCount");
		AutoGiveItem(dwSelectedItemIdx, dwSelectedItemCount);

		ChatPacket(CHAT_TYPE_COMMAND, "ShowWhellReward %u %u", dwSelectedItemIdx, dwSelectedItemCount);
		SetProtectTime("WheelIndex", 0);
		SetProtectTime("WheelLuckyIndex", 0);
		SetProtectTime("WheelLuckyItemVnum", 0);
		SetProtectTime("WheelLuckyItemCount", 0);
		SetProtectTime("WheelWorking", 0);
	}
#endif
	if (GetShop())
	{
...
#ifdef ENABLE_PVP_RANKING
	if (GetMapIndex() == CPvPRanking::instance().GetMapIndex() && quest::CQuestManager::instance().GetEventFlag ("pvp_ranking") < 1)
	{
		SetRankKill (0);
		SetRankDead (0);
	}
...
#ifdef ENABLE_WINGS_NEW
	SetPart(PART_WING, t->parts[PART_WING]);
#endif
	m_points.iRandomHP = t->sRandomHP;
	m_points.iRandomSP = t->sRandomSP;

#ifdef ENABLE_PVP_RANKING
	m_points.rank_Kill = t->rank_Kill;
	m_points.rank_Dead = t->rank_Dead;
#endif
	// REMOVE_REAL_SKILL_LEVLES
	if (m_pSkillLevels)
		M2_DELETE_ARRAY(m_pSkillLevels);
...
#endif
#ifdef ENABLE_WINGS_NEW
	SetPart(PART_WING, GetOriginalPart(PART_WING));
#endif
...
#endif
{
	...
	if (val > max_val)
#ifdef on
		sys_err("POINT_ERROR: %s type %d val %lld (max: %lld)", GetName(), type, val, max_val);//@hinata18
#else
...
#ifdef on
	int64_t len = snprintf(chatbuf, sizeof(chatbuf), "%llu)", (DWORD)GetVID());
#else
	int len = snprintf(chatbuf, sizeof(chatbuf), "%u)", (DWORD)GetVID());
#endif

#ifdef on
	if (len < 0 || len >= (int64_t)sizeof(chatbuf))
#else
...
#ifdef on
	int64_t len2 = vsnprintf(chatbuf + len, sizeof(chatbuf) - len, format, args);
#else
	int len2 = vsnprintf(chatbuf + len, sizeof(chatbuf) - len, format, args);
#endif

#ifdef on
	if (len2 < 0 || len2 >= (int64_t)sizeof(chatbuf) - len)
#else
...
DWORD CHARACTER::GetPart(BYTE bPartPos) const // @fixme502
{
	assert(bPartPos < PART_MAX_NUM);
#ifdef ENABLE_WINGS_NEW
	if (bPartPos == PART_WING)
	{
		const LPITEM pWing = GetWear(WEAR_COSTUME_WING);
		if (pWing)
			return pWing->GetVnum();
	}
#endif
...
void CHARACTER::OnAlignUpdate(BYTE now_align_index)
{
	if (now_align_index == 0)
	{
		...
		AddAffect(AFFECT_ALIGN_BONUS1, POINT_ATTBONUS_MONSTER, 500, 0, 60*60*60*365, 0, true);
		AddAffect(AFFECT_ALIGN_BONUS2, POINT_ATTBONUS_STONE, 500, 0, 60*60*60*365, 0, true);
		AddAffect(AFFECT_ALIGN_BONUS3, POINT_ATTBONUS_BOSS, 300, 0, 60*60*60*365, 0, true);
	}
	else if (now_align_index >= 1 && now_align_index <= 40)
	{
		...
		int align_bonus = (41 - now_align_index) * 10;

		AddAffect(AFFECT_ALIGN_BONUS1, POINT_ATTBONUS_MONSTER, align_bonus, 0, 60*60*60*365, 0, true);
		AddAffect(AFFECT_ALIGN_BONUS2, POINT_ATTBONUS_STONE, align_bonus, 0, 60*60*60*365, 0, true);
...
bool CHARACTER::UpdateCoins (DWORD dwAID, long val)
{
	...
	if (val > 0)
	{
		sprintf (szQuery, "UPDATE account.account SET cash = cash + '%ld' WHERE id = '%u'", lValue, dwAID);
	}
	else
	{
		sprintf (szQuery, "UPDATE account.account SET cash = cash - '%ld' WHERE id = '%u'", lValue, dwAID);
...
bool CHARACTER::UpdateVoteCoins (DWORD dwAID, long val)
{
	...
	if (val > 0)
	{
		sprintf (szQuery, "UPDATE account.account SET mileage = mileage + '%ld' WHERE id = '%u'", lValue, dwAID);
	}
	else
	{
		sprintf (szQuery, "UPDATE account.account SET mileage = mileage - '%ld' WHERE id = '%u'", lValue, dwAID);
