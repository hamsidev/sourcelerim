...
void CActorInstance::AttachWing(CItemData* pItemData, float fSpecular)
{
	if (!pItemData)
	{
	...
	RegisterModelThing(CRaceData::PART_WING, pItemData->GetModelThing());
	SetModelInstance(CRaceData::PART_WING, CRaceData::PART_WING, 0);
	AttachModelInstance(CRaceData::PART_MAIN, "Bip01 Spine2", CRaceData::PART_WING);
	...
	if (fSpecular > 0.0f)
	{
		...
		kMaterialData.bSphereMapIndex = 1;
		SetMaterialData(CRaceData::PART_WING, NULL, kMaterialData);
	}

	/* if (CGrannyLODController* pLODController = m_LODControllerVector[CRaceData::PART_WING])
	{
