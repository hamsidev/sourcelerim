...
LPITEM CHARACTER::GetItem(TItemPos Cell) const
{
	...
	case DRAGON_SOUL_INVENTORY:
		...
		case GENERAL_INVENTORY:
			...
			return m_PlayerSlots->pSSGItems[wCell];
		case BONUS_INVENTORY:
			if (wCell >= SPECIAL_INVENTORY_MAX_NUM)
			{
				sys_err("CHARACTER::GetInventoryItem: invalid SSBO item cell %d", wCell);
				return NULL;
			}
			return m_PlayerSlots->pSSBOItems[wCell];
#endif
...
)
{
	...
	case DRAGON_SOUL_INVENTORY:
		...
		case UPGRADE_INVENTORY:
			{
				LPITEM pOld = m_PlayerSlots->pSSUItems[wCell];
		...
		case BOOK_INVENTORY:
			{
				LPITEM pOld = m_PlayerSlots->pSSBItems[wCell];
		...
		case STONE_INVENTORY:
			{
				LPITEM pOld = m_PlayerSlots->pSSSItems[wCell];
		...
		case GENERAL_INVENTORY:
			{
				LPITEM pOld = m_PlayerSlots->pSSGItems[wCell];
				...
				if (pOld)
				{
					if (wCell < SPECIAL_INVENTORY_MAX_NUM)
					{
						for (int i = 0; i < pOld->GetSize(); ++i)
						{
							...
							if (m_PlayerSlots->pSSGItems[p] && m_PlayerSlots->pSSGItems[p] != pOld)
								continue;

							m_PlayerSlots->wSSGItemGrid[p] = 0;
						}
					}
					else
						m_PlayerSlots->wSSGItemGrid[wCell] = 0;
				}
				...
				if (pItem)
				{
					...
					if (wCell < SPECIAL_INVENTORY_MAX_NUM)
					{
						for (int i = 0; i < pItem->GetSize(); ++i)
						{
							...
							m_PlayerSlots->wSSGItemGrid[p] = wCell + 1;
						}
					}
					else
						m_PlayerSlots->wSSGItemGrid[wCell] = wCell + 1;
				}

				m_PlayerSlots->pSSGItems[wCell] = pItem;
			}
			break;
		case BONUS_INVENTORY:
			{
				LPITEM pOld = m_PlayerSlots->pSSBOItems[wCell];
				...
				if (pOld)
				{
					if (wCell < SPECIAL_INVENTORY_MAX_NUM)
					{
						for (int i = 0; i < pOld->GetSize(); ++i)
						{
							...
							if (m_PlayerSlots->pSSBOItems[p] && m_PlayerSlots->pSSBOItems[p] != pOld)
								continue;

							m_PlayerSlots->wSSBOItemGrid[p] = 0;
						}
					}
					else
						m_PlayerSlots->wSSBOItemGrid[wCell] = 0;
				}
				...
				if (pItem)
				{
					...
					if (wCell < SPECIAL_INVENTORY_MAX_NUM)
					{
						for (int i = 0; i < pItem->GetSize(); ++i)
						{
							...
							m_PlayerSlots->wSSBOItemGrid[p] = wCell + 1;
						}
					}
					else
						m_PlayerSlots->wSSBOItemGrid[wCell] = wCell + 1;
				}

				m_PlayerSlots->pSSBOItems[wCell] = pItem;
			}
			break;
#endif
...
void CHARACTER::ClearItem()
{
	...
	{
		int j;
		for (j = 0; j < SPECIAL_INVENTORY_MAX_NUM; ++j)
		{
			if ((item = GetItem(TItemPos(GetSpecialWindowType(i), j))))
			{
				item->SetSkipSave(true);
				ITEM_MANAGER::instance().FlushDelayedSave(item);
		...
		case UPGRADE_INVENTORY:
			{
				...
				if (m_PlayerSlots->wSSUItemGrid[bCell])
				{
					if (m_PlayerSlots->wSSUItemGrid[bCell] == iExceptionCell)
					{
						if (bSize == 1)
							return true;
						...
						WORD bPage = bCell / (SPECIAL_INVENTORY_MAX_NUM / SPECIAL_INVENTORY_PAGE_COUNT);
		...
		case BOOK_INVENTORY:
			{
				...
				if (m_PlayerSlots->wSSBItemGrid[bCell])
				{
					if (m_PlayerSlots->wSSBItemGrid[bCell] == iExceptionCell)
					{
						if (bSize == 1)
							return true;
		...
		case STONE_INVENTORY:
			{
				...
				if (m_PlayerSlots->wSSSItemGrid[bCell])
				{
					if (m_PlayerSlots->wSSSItemGrid[bCell] == iExceptionCell)
					{
						if (bSize == 1)
							return true;
		...
		case GENERAL_INVENTORY:
			{
				...
				if (m_PlayerSlots->wSSGItemGrid[bCell])
				{
					if (m_PlayerSlots->wSSGItemGrid[bCell] == iExceptionCell)
					{
						if (bSize == 1)
							return true;
						...
						do
						{
							...
							if (m_PlayerSlots->wSSGItemGrid[p])
								if (m_PlayerSlots->wSSGItemGrid[p] != iExceptionCell)
									return false;
						}
						while (++j < bSize);
		...
		case BONUS_INVENTORY:
			{
				...
				if (m_PlayerSlots->wSSBOItemGrid[bCell])
				{
					if (m_PlayerSlots->wSSBOItemGrid[bCell] == iExceptionCell)
					{
						if (bSize == 1)
							return true;
						...
						do
						{
							...
							if (m_PlayerSlots->wSSBOItemGrid[p])
								if (m_PlayerSlots->wSSBOItemGrid[p] != iExceptionCell)
									return false;
						}
						while (++j < bSize);
...
#ifdef SANDIK_AC
int CHARACTER::GetEmptyAll(BYTE bSize, BYTE bWindow) const
{
...
int CHARACTER::GetEmptyAll(LPITEM pItem) const
{
	if (pItem == NULL)
		return -1;
	
	// if (pItem->IsDragonSoul())
		// return GetEmptyDragonSoulInventory(pItem);
	...
	if (pItem->IsUpgradeItem())
		...
		bWindow = GENERAL_INVENTORY;
	else
		bWindow = INVENTORY;
		
	for (int i = 0; i < SPECIAL_INVENTORY_MAX_NUM; i++)
	{
		if (IsEmptyItemGrid(TItemPos(bWindow, i), pItem->GetSize()))
			return i;
	}
	return -1;
}
					...
					case USE_SPECIAL:
						...
						switch (item->GetVnum())
						{
							...
							case REWARD_BOX_UNIQUE_ITEM_CAPE_OF_COURAGE:
								AggregateMonster();
								// item->SetCount(item->GetCount()-1); @hinata36
								break;
						...
						case 70072:
								{
									...
									if (get_global_time() - last_use_time < interval * 60 * 0)
									{
										if (test_server == false)
										{
											ChatPacket(CHAT_TYPE_INFO, LC_TEXT("¾ÆÁ÷ »ç¿ëÇÒ ¼ö ¾ø½À´Ï´Ù."));
									...
									if (GetAlignment() >= 30000000) // Derece 30000000 ise item kullanýlmasýn
								#else
									if (GetAlignment() >= 200000)
								#endif
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("3M KNK BITTI SAKIN OL"));
										return false;
									}
								
								#ifdef ENABLE_ALIGN_RENEWAL
									if (30000001 - GetAlignment() < val * 10)
									{
										val = (30000001 - GetAlignment()) / 10;
									}
								#else
									if (200000 - GetAlignment() < val * 10)
									{
							...
							case ITEM_GIVE_STAT_RESET_COUNT_VNUM:
								{
#ifdef ENABLE_STAT_COPY_BUG_FIX
							...
							case 77666 : // ???
								{
									...
									if (item2->GetAttributeValue(5) >= 50)
									{
										ChatPacket(CHAT_TYPE_INFO, "Bu nesneye en fazla 50 bonus getirebilirsin !");
										return false;
									}
...
#ifdef ENABLE_STONE_ADD
							case 281123 : //Silah Matkabý
								{
									LPITEM item2;
									...
									if (item2->GetType() == ITEM_WEAPON || item2->GetType() == ITEM_ARMOR || item2->GetType() == ITEM_BELT)
									{
										...
										if (*szEventFlag)
										{
											...
											{
												ChatPacket(CHAT_TYPE_INFO, "|cff00ff00Sistem: |cff00ccffKilitli itemin üstüne matkap kullanamazin.");
												return false;
											}
										}
									}
									...
									if ((item2->GetType() == ITEM_WEAPON))
									{
										...
										{
											ChatPacket(CHAT_TYPE_INFO, "|cff00ff00Sistem: |cff00ccffBu silah'a 4. Taþ Slotundan fazla slot ekleyemezsin.");
											return false;
										}
										if (item2->GetSubType() == WEAPON_ARROW)
										{
											ChatPacket(CHAT_TYPE_INFO, "|cff00ff00Sistem: |cffffff00Silah Matkabý ile silah'tan baþka bir iteme Taþ Slotu ekleyemezsin.");
											return false;
										}
										if (item2->GetSocketCount() < 3) /// buradan soketleri ayarlayabilirsiniz...
										{
											ChatPacket(CHAT_TYPE_INFO, "|cff00ff00Sistem: |cffffff00Bu silah'a Taþ Slotu açýlamaz. Taþ Slotu açýlabilir olmasý için 3 adet Taþ Slotu olmalýdýr.");
											return false;
										}
										item2->AddSocket();
										item2->UpdatePacket();
										item2->Save();
										ChatPacket(CHAT_TYPE_INFO, "|cff00ff00Sistem: |cff00C78CSilah'ýna 4. Taþ Slotu basarýyla eklendi.");
										item->SetCount(item->GetCount() - 1);
									}
									else
									{
										ChatPacket(CHAT_TYPE_INFO, "|cff00ff00Sistem: |cffffff00Silah Matkabý silah'tan baþka bir iteme Taþ Slotu eklemez!.");
										break;
									}
								}
								break;
								
							case 281124 : //Zýrh Matkabý
								{
									LPITEM item2;
									...
									if ((item2->GetType() == ITEM_ARMOR && item2->GetSubType() == ARMOR_BODY))
									{
										if (item2->GetSocketCount() == ITEM_SOCKET_MAX_NUM)
										{
											ChatPacket(CHAT_TYPE_INFO, "|cff00ff00Sistem: |cff00ccffBu zýrh'a 4. Taþ Slotundan fazla Taþ Slotu ekleyemezsin.");
											return false;
										}
										if (item2->GetSubType() == WEAPON_ARROW)
										{
											ChatPacket(CHAT_TYPE_INFO, "|cff00ff00Sistem: |cffffff00Zýrh Matkabý ile zýrh'tan baþka bir iteme Taþ Slotu ekleyemezsin.");
											return false;
										}
										if (item2->GetSocketCount() < 3) //// buradan soketleri ayarlayabilirsiniz
										{
											ChatPacket(CHAT_TYPE_INFO, "|cff00ff00Sistem: |cffffff00Bu zýrh'a Taþ Slotu açýlamaz. Taþ Slotu açýlabilir olmasý için 3 adet Taþ Slotu olmalýdýr.");
											return false;
										}
										item2->AddSocket();
										item2->UpdatePacket();
										item2->Save();
										ChatPacket(CHAT_TYPE_INFO, "|cff00ff00Sistem: |cff00C78CZýrhýna 4. Taþ Slotu basarýyla eklendi.");
										item->SetCount(item->GetCount() - 1);
									}
									else
									{
										ChatPacket(CHAT_TYPE_INFO, "|cff00ff00Sistem: |cffffff00Zýrh Matkabý zýrh'tan baþka bir iteme Taþ Slotu eklemez!.");
										break;
									}
								}
								break;
					...
					case USE_ABILITY_UP:
						{
							switch (item->GetValue(0))
							{
								case APPLY_MOV_SPEED:
									...
									if (FindAffect(AFFECT_MOV_SPEED))//@hinata15
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ÀÌ¹Ì È¿°ú°¡ °É·Á ÀÖ½À´Ï´Ù."));
										return false;
									}
								...
								case APPLY_ATT_SPEED:
									...
									if (FindAffect(AFFECT_ATT_SPEED))//@hinata15
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ÀÌ¹Ì È¿°ú°¡ °É·Á ÀÖ½À´Ï´Ù."));
										return false;
									}
					...
					case USE_ADD_ATTRIBUTE2:
						{
							...
							switch (item->GetSubType())
							{
								...
								case USE_CHANGE_ATTRIBUTE2 : // @fixme123
...
#ifdef ORT_BEC_EFSUN_SYSTEM
									else if (item->GetVnum() == 75888)
									{
										if ((item2-> GetType() == ITEM_WEAPON) && (item2->HasAttr(72)) )
										{
											int32_t value1 = number(45, 60);
											if (item2->GetAttributeValue(0) >= 60)
											{
												ChatPacket(CHAT_TYPE_INFO, "Bu nesneye en fazla 60 Ortalama getirebilirsin !");
												return false;
											}
										...
										{
											ChatPacket(CHAT_TYPE_INFO, "Bu esyayi sadece Ortalama ve Becerili silahlarda kullanabilirsiniz.");
											return false;
										}
									}
									...
									{
										if ((item2-> GetType() == ITEM_WEAPON) && (item2->HasAttr(71)) )
										{
											int32_t value1 = -(number(23, 30));

											if (item2->GetAttributeValue(1) >= 30)
											{
												ChatPacket(CHAT_TYPE_INFO, "Bu nesneye en fazla 30 Beceri getirebilirsin !");
												return false;
											}

											int32_t value2 = number(20,30);
											item2->SetForceAttribute(0, 72, value1);
											item2->SetForceAttribute(1, 71, value2);
										}
		...
		case ITEM_METIN:
			{
				...
				for (i = 0; i < ITEM_SOCKET_MAX_NUM; ++i)
					if (item2->GetSocket(i) >= 1 && item2->GetSocket(i) <= 2 && item2->GetSocket(i) >= item->GetValue(2))
					{
						...
						item->SetCount(item->GetCount() - 1);//@hinata20
						break;
					}
...
//@hinata60
 // dupe fix
	...
	else
	{
		...
		if (!item->IsGeneralItem() && GENERAL_INVENTORY == DestCell.window_type)
			return false;
	...
	if (item->DistanceValid(this))
	{
		...
		if (item->IsOwnership(this))
		{
			...
			else
			{
				...
				if (finalItem)
				{
					ChatPacket(CHAT_TYPE_INFO, LC_TEXT("¾ÆÀÌÅÛ È¹µæ: %s"), finalItem->GetName());
...
bool CHARACTER::EquipItem(LPITEM item, int iCandidateCell)
{
	...
	if (iWearCell == WEAR_BODY && IsRiding() && (item->GetVnum() >= 11902 && item->GetVnum() <= 11911 && item->GetVnum() <= 11912)) //Damatlýklar
	{
...
#ifdef ENABLE_FAST_EQUIP_FIX
	int iPulse = thecore_pulse();
#endif

#ifdef ENABLE_FAST_EQUIP_FIX
		if (iPulse - GetLastItemFastWear() < passes_per_sec * 1 &&
		(iWearCell == WEAR_BODY ||
		iWearCell == WEAR_HEAD ||
		iWearCell == WEAR_FOOTS ||
		iWearCell == WEAR_WRIST ||
		iWearCell == WEAR_WEAPON ||
		iWearCell == WEAR_NECK ||
		iWearCell == WEAR_EAR ||
		iWearCell == WEAR_SHIELD)
		)
		{
		ChatPacket(CHAT_TYPE_INFO, "Bunu yapabilmek için 1 saniye beklemelisin.");
		return false;
		}
#endif
...
// #ifdef __PET_SYSTEM__
		// else if (COSTUME_PET == item->GetSubType() && item->GetType() == ITEM_COSTUME)
		// {
			...
			// {
				...
				// {
					// if (quest::CQuestManager::instance().GetEventFlagBR("pet", GetGuild()->GetName()) > 0)
					// {
						// ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Bu savasta PET kullanimi engellendi."));
						// return false;
					// }
				// }
			// }
...
LPITEM CHARACTER::FindSpecifyItem(DWORD vnum) const
{
	...
	for (int i = 0; i < 5; ++i)
	{
		for (int j = 0; j < SPECIAL_INVENTORY_MAX_NUM; ++j)
			if (GetSpecialStorageItem(j, GetSpecialWindowType(i)) && GetSpecialStorageItem(j, GetSpecialWindowType(i))->GetVnum() == vnum)
				return GetSpecialStorageItem(j, GetSpecialWindowType(i));
	}
#endif
...
LPITEM CHARACTER::FindItemByID(DWORD id) const
{
	...
	{
		for (int j = 0; j < SPECIAL_INVENTORY_MAX_NUM; ++j)
			if (NULL != GetSpecialStorageItem(j, GetSpecialWindowType(i)) && GetSpecialStorageItem(j, GetSpecialWindowType(i))->GetID() == id)
				return GetSpecialStorageItem(j, GetSpecialWindowType(i));
	}
#endif
...
int CHARACTER::CountSpecifyItem(DWORD vnum) const
{
	...
	for (int i = 0; i < 5; ++i)
	{
		for (int j = 0; j < SPECIAL_INVENTORY_MAX_NUM; ++j)
		{
			item = GetSpecialStorageItem(j, GetSpecialWindowType(i));
			if (NULL != item && item->GetVnum() == vnum)
			{
				if (m_pkMyShop && m_pkMyShop->IsSellingItem(item->GetID()))
					continue;
...
void CHARACTER::RemoveSpecifyItem(DWORD vnum, DWORD count)
{
	...
	for (int i = 0; i < 5; ++i)
	{
		for (UINT j = 0; j < SPECIAL_INVENTORY_MAX_NUM; ++j)
		{
			LPITEM item = GetSpecialStorageItem(j, GetSpecialWindowType(i));
...
std::pair<bool, LPITEM> CHARACTER::AutoStackItemProto(DWORD dwItemVnum
#ifdef ENABLE_ITEM_COUNT_LIMIT_SYSTEM
, DWORD & bCount
#else
, BYTE & bCount
	...
	if (item->IsSpecialStorageItem() && item->IsStackable() && !IS_SET(item->GetAntiFlag(), ITEM_ANTIFLAG_STACK))
	{
		...
		for (int i = 0; i < SPECIAL_INVENTORY_MAX_NUM; ++i)
		{
			...
			if (item2->GetVnum() == item->GetVnum())
			{
				...
				if (bCount == 0)
				{
					ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Item recieved in special storage: %s."), item2->GetName());
					M2_DESTROY_ITEM(item);
					return item2;
				}
			}
...
)
{
	...
	{
		if (bMsg)
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("¾ÆÀÌÅÛ È¹µæ: %s"), foundItem->GetName());
		return foundItem;
	}
			...
			if (inv_item->GetType() == ITEM_BLEND)
			{
				if (inv_item->GetVnum() == item->GetVnum())
				{
					if (inv_item->GetSocket(0) == item->GetSocket(0) &&
							...
							// inv_item->GetCount() < g_bItemCountLimit) @hinata50
							inv_item->GetCount() + item->GetCount() <= g_bItemCountLimit)//@hinata50
					{
						inv_item->SetCount(inv_item->GetCount() + item->GetCount());
						M2_DESTROY_ITEM(item);//@hinata12
						return inv_item;
					}
				}
			}
...
#ifdef SANDIK_AC
bool CHARACTER::UseItemAll(TItemPos Cell)
{
	if (quest::CQuestManager::instance().GetEventFlag("instant_chest_disable") == 1)
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Sistem> Bu islem suanlik devredisi"));
		return false;
	}
	
	if (GetMapIndex() == 3 || GetMapIndex() == 23 || GetMapIndex() == 43 || GetMapIndex() == 59 || GetMapIndex() == 370 || GetMapIndex() == 364 || GetMapIndex() >= 1000)
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("KOYLERDE_SANDIK"));
		return false;
	}
	...
	if (!IsValidItemPosition(Cell) || !(pkItem = GetItem(Cell)))
		return false;
	
	if (pkItem->isLocked())
		return false;
	
	if (!pkItem->IsGeneralItem())
		return false;
	
	if (pkItem->IsExchanging())
		return false;
	
	if (!pkItem->CanUsedBy(this))
		return false;
	
	if (false == FN_check_item_sex(this, pkItem))
		return false;
	SetExchangeTime();
	
	int itemCount = pkItem->GetCount();
	...
	const CSpecialItemGroup * pGroup = ITEM_MANAGER::instance().GetSpecialItemGroup(pkItem->GetVnum());
	...
	while (itemCount--)
	{
		...
		pkItem->SetCount(pkItem->GetCount() - 1, (itemCount < 1) ? false : true);
		std::vector <int> idxes;
		int n = pGroup->GetMultiIndex(idxes);
		...
		for (int i = 0; i < n; i++)
		{
			...
			{
				itertype(mGiveList) it = mGiveList.find(dwVnum);
				if (it == mGiveList.end())
					mGiveList.insert(std::make_pair(dwVnum, dwCount));
				else
					it->second += dwCount;
			}
			else
			{
				...
				if (pTable->IsUpgradeItem())
					iEmptyCell = GetEmptyAll(pTable->GetSize(), UPGRADE_INVENTORY);
				else if (pTable->IsBook())
					iEmptyCell = GetEmptyAll(pTable->GetSize(), BOOK_INVENTORY);
				else if (pTable->IsStone())
					iEmptyCell = GetEmptyAll(pTable->GetSize(), STONE_INVENTORY);
				else if (pTable->IsGeneralItem())
					iEmptyCell = GetEmptyAll(pTable->GetSize(), GENERAL_INVENTORY);
				else
					iEmptyCell = GetEmptyAll(pTable->GetSize(), INVENTORY);
				
				if (iEmptyCell != -1)
				{
					LPITEM pkGift = ITEM_MANAGER::Instance().CreateItem(dwVnum, dwCount);
					
					if (!pkGift)
						return false;
					
					if (pkGift->IsUpgradeItem())
						pkGift->AddToCharacter(this, TItemPos(UPGRADE_INVENTORY, iEmptyCell));
					else if (pkGift->IsBook())
						pkGift->AddToCharacter(this, TItemPos(BOOK_INVENTORY, iEmptyCell));
					else if (pkGift->IsStone())
						pkGift->AddToCharacter(this, TItemPos(STONE_INVENTORY, iEmptyCell));
					else if (pkGift->IsGeneralItem())
						pkGift->AddToCharacter(this, TItemPos(GENERAL_INVENTORY, iEmptyCell));
					else if (pkGift->IsDragonSoul())
						pkGift->AddToCharacter(this, TItemPos(DRAGON_SOUL_INVENTORY, iEmptyCell));
					else
						pkGift->AddToCharacter(this, TItemPos(INVENTORY, iEmptyCell));
					ChatPacket(CHAT_TYPE_INFO, "%d adet %s kazandiniz.", pkGift->GetCount(), pkGift->GetName());
				}
				else
				{
					if (pkItem)
	...
	for (itertype(mGiveList) it = mGiveList.begin(); it != mGiveList.end(); it++)
	{
		DWORD dwVnum	= it->first;
		DWORD iCount	= it->second;
		
		if (iCount >= ITEM_MAX_COUNT)
		{
			do
			{
				DWORD wCount = MIN(iCount, ITEM_MAX_COUNT);
				iCount -= wCount;
				AutoGiveItem(dwVnum, wCount);
			}
			while (iCount != 0);
		}
		else
			AutoGiveItem(dwVnum, iCount);
		TItemTable * pTable = ITEM_MANAGER::Instance().GetTable(it->first);
		if (!pTable)
			return false;
		ChatPacket(CHAT_TYPE_INFO, "%d adet %s kazandiniz.", it->second, pTable->szLocaleName);
	}
	mGiveList.clear();
	return true;
}
...
DWORD CHARACTER::GetSpecialWindowType(DWORD eType) const
{
	DWORD window[5] = {UPGRADE_INVENTORY, BOOK_INVENTORY, STONE_INVENTORY, GENERAL_INVENTORY, BONUS_INVENTORY};
...
LPITEM CHARACTER::GetGeneralInventoryItem(WORD wCell) const
{
	return GetItem(TItemPos(GENERAL_INVENTORY, wCell));
}
