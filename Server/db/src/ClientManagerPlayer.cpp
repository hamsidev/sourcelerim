	...
	if ((c = GetPlayerCache(packet->player_id)))
	{
		...
		else
		{
			snprintf(szQuery, sizeof(szQuery),
					...
					"FROM item%s WHERE owner_id=%d AND (`window` in ('INVENTORY','EQUIPMENT','DRAGON_SOUL_INVENTORY','UPGRADE_INVENTORY','BOOK_INVENTORY','STONE_INVENTORY','GENERAL_INVENTORY','BONUS_INVENTORY','BELT_INVENTORY'))", // ENABLE_SPECIAL_STORAGE
					GetTablePostfix(), pTab->id);
...
#endif
#ifdef ENABLE_WINGS_NEW
	str_to_number(pkTab->parts[PART_WING], row[col++]);
#endif
#ifdef ENABLE_PVP_RANKING
	str_to_number (pkTab->rank_Kill, row[col++]);
	str_to_number (pkTab->rank_Dead, row[col++]);
#endif
...
void CClientManager::__RESULT_PLAYER_DELETE(CPeer *peer, SQLMsg* msg)
{
	...
	if (msg->Get() && msg->Get()->uiNumRows)
	{
		...
		snprintf(queryStr, sizeof(queryStr), "DELETE FROM item%s WHERE owner_id=%d AND (`window` in ('INVENTORY','EQUIPMENT','DRAGON_SOUL_INVENTORY','UPGRADE_INVENTORY','BOOK_INVENTORY','STONE_INVENTORY','GENERAL_INVENTORY','BONUS_INVENTORY','BELT_INVENTORY'))", GetTablePostfix(), pi->player_id); // ENABLE_SPECIAL_STORAGE
		CDBManager::instance().DirectQuery(queryStr);
