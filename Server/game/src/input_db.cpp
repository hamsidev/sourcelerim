...
void CInputDB::Boot(const char* data)
{
	...
	snprintf(szCommonDropItemFileName, sizeof(szCommonDropItemFileName),
		"%s/whell_drop_data.txt", LocaleService_GetBasePath().c_str());
	if (!ITEM_MANAGER::instance().ReadWhellRewards(szCommonDropItemFileName))
	{
		sys_err("cannot load ReadWhellRewards: %s", szCommonDropItemFileName);
		thecore_shutdown();
		return;
	}
#endif
...
void CInputDB::GuildRonarkKing(const char* c_pData)
{
	TPacketDGGuildRonarkKing* p = (TPacketDGGuildRonarkKing*)c_pData;

	CGuild* g = CGuildManager::instance().TouchGuild(p->dwGuild);
	if (g)
	{
		g->RecvRonarkKing(p->ronark_king);
	}
}
#endif
...
int CInputDB::Analyze(LPDESC d, BYTE bHeader, const char * c_pData)
{
	...
	case HEADER_DG_RESPOND_CHANNELSTATUS:
		...
		break;
#ifdef ENABLE_RONARK_SYSTEM
	case HEADER_DG_GUILD_RONARK_KING:
		GuildRonarkKing(c_pData);
		break;
#endif
