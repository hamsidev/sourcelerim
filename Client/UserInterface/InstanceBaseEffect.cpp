...
#endif
{
	if (CPythonSystem::Instance().IsShowDamage())
	{
		...
		if (m_DamageQueue.size() > 25) // @fixme025
			m_DamageQueue.pop();
	}
}
	...
	*/
	...
	while(damage>0)
	{
		...
		if (damage > 0 && (index % 3) == 0) {
            ...
            if (valIdx > 0) {
                if (valIdx > 3)
                    textures.push_back("d:/ymir work/effect/affect/damagevalue/damage_s.dds");
				else if (valIdx > 2)
                    textures.push_back("d:/ymir work/effect/affect/damagevalue/damage_kt.dds"); //sorun olmaz eypcn
                else if (valIdx > 1)
