...
/*RONARK*/
		// { pkBase.EFFECT_AFFECT + 46, "Bip01", "d:/ymir work/effect/ronark/ronark_hukumdari_zency.mse"},
/*RONARK*/
...
const bool CPlayerSettingsModule::LoadGameWarrior(DWORD dwRace, const char* stFolder)
{
	CRaceManager& pkManager = CRaceManager::Instance();
...
#ifdef ENABLE_ACCE_COSTUME_SYSTEM
	pRaceData->RegisterAttachingBoneName(CRaceData::PART_ACCE, "Bip01");
#endif

	return true;
}

const bool CPlayerSettingsModule::LoadGameAssassin(DWORD dwRace, const char* stFolder)
{
	CRaceManager& pkManager = CRaceManager::Instance();
	pkManager.SelectRace(dwRace);
...
const bool CPlayerSettingsModule::LoadGameSura(DWORD dwRace, const char* stFolder)
{
	CRaceManager& pkManager = CRaceManager::Instance();
	pkManager.SelectRace(dwRace);
...
const bool CPlayerSettingsModule::LoadGameShaman(DWORD dwRace, const char* stFolder)
{
	CRaceManager& pkManager = CRaceManager::Instance();
	pkManager.SelectRace(dwRace);
...
#ifdef ENABLE_WOLFMAN_CHARACTER
const bool CPlayerSettingsModule::LoadGameWolfman(DWORD dwRace, const char* stFolder)
{
	CRaceManager& pkManager = CRaceManager::Instance();
	pkManager.SelectRace(dwRace);
