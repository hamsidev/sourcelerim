...
bool CExchange::CheckSpecialStorageSpace(BYTE bStorageType)
{
	...
	for (i = 0; i < SPECIAL_INVENTORY_PAGE_SIZE*1; ++i)
	{
		...
		item = victim->GetSpecialStorageItem(i, victim->GetSpecialWindowType(bStorageType));
	...
	for (i = 0; i < EXCHANGE_ITEM_MAX_NUM; ++i)
	{
		...
		if((bStorageType == 0 && (!item->IsUpgradeItem())) || (bStorageType == 1 && (!item->IsBook())) || (bStorageType == 2 && (!item->IsStone())) || (bStorageType == 3 && (!item->IsGeneralItem())) || (bStorageType == 4 && (!item->IsBonus())))
			continue;
...
bool CExchange::Accept(bool bAccept)
{
	...
	if (m_bAccept && GetCompany()->m_bAccept)
	{
		...
		for(int i = 0; i < 5; i++)
		{
			...
			{
				victim->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("|TNot enough Gold or no space in the inventory.|t"));
				GetOwner()->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("|TNot enough Gold or no space in the inventory.|t"));
				goto EXCHANGE_END;
			}
			
			if (!GetCompany()->CheckSpecialStorageSpace(i))
			{
				victim->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("|TThe other person has no space in the inventory left.|t"));
				GetOwner()->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("|TThere isn't enough space in the inventory.|t"));
				goto EXCHANGE_END;
			}
		}
#endif
