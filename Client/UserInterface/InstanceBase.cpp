...
bool CInstanceBase::Create(const SCreateData& c_rkCreateData)
{
	...
	if (IsPC())
	{
		...
		if (strMapName == "metin2_map_t1") // alan savasi map degistir
		{
			RefreshTextTail();
		}
#endif
		...
		SetWing(c_rkCreateData.m_dwWing);
#endif
...
void CInstanceBase::SetWing(DWORD dwWing)
{
	...
	if (dwWing == 0)
	{
		m_awPart[CRaceData::PART_WING] = 0;
		m_GraphicThingInstance.AttachWing(0, 0.0f);
		ClearWingEffect();
#ifdef ENABLE_SHINING_SYSTEM
		__ClearWingShiningEffect();
#endif
	...
	m_awPart[CRaceData::PART_WING] = dwWing;

	CItemData* pItemData = NULL;
	CItemManager::Instance().GetItemDataPointer(dwWing, &pItemData);
#ifdef ENABLE_SHINING_SYSTEM
	if (pItemData)
		__GetShiningEffect(pItemData);
#endif
	m_GraphicThingInstance.AttachWing(pItemData);
...
#ifdef ENABLE_WYNEE_CAPESYSTEM //Cape
void CInstanceBase::SetCape(DWORD dwCape)
{
	if (!IsPC())
		return;
	...
	float fSpecular = 30.0f;

	fSpecular /= 100.0f;

	m_awPart[CRaceData::PART_CAPE] = dwCape;

	CItemData* pItemData = NULL;
	CItemManager::Instance().GetItemDataPointer(dwCape, &pItemData);
	
	m_GraphicThingInstance.AttachCape(pItemData, fSpecular);
#ifdef ENABLE_OBJ_SCALLING
	DWORD dwRace = GetRace(), dwPos = RaceToJob(dwRace), dwSex = RaceToSex(dwRace);
	dwPos += 1;
	if (dwSex == 0)
...
void CInstanceBase::ChangeCape(DWORD dwCape)
{
	if (!IsPC())
		return;
...
void CInstanceBase::ClearCapeEffect()
{
	if (!m_dwCapeEffect)
		return;

	__EffectContainer_DetachEffect(m_dwCapeEffect);
	m_dwCapeEffect = 0;
}
#endif
...
void CInstanceBase::ClearWingEffect()
{
	if (!m_dwWingEffect)
		return;

	__EffectContainer_DetachEffect(m_dwWingEffect);
	m_dwWingEffect = 0;
}
#endif
	...
	if (pItem->GetSubType() == CItemData::COSTUME_BODY)
	{
		...
		if (pItem->GetSubType() == CItemData::COSTUME_ACCE)
		{
			for (int i = 0; i < CItemData::ITEM_SHINING_MAX_COUNT; i++)
			{
				...
				{
					__AttachAcceShiningEffect(i, shiningTable.szShinings[i], "Bip01"); // orjinal Bip01 Spine2
				}
				else
				{
...
#endif
#ifdef ENABLE_WINGS_NEW
		else if (pItem->GetSubType() == CItemData::COSTUME_WING)
		{
			__ClearWingShiningEffect();
			for (int i = 0; i < CItemData::ITEM_SHINING_MAX_COUNT; i++)
...
/* #ifdef ENABLE_ACCE_COSTUME_SYSTEM
    if (pItem->GetSubType() == CItemData::COSTUME_ACCE)
    {
        for (int i = 0; i < CItemData::ITEM_SHINING_MAX_COUNT; i++)
        {
            // Eğer pItem'in index değeri 86066 ile 86080 arasında ise, "Bip01" veya "Bip01 Spine2" efektini kontrol et
            if (pItem->GetIndex() >= 86066 && pItem->GetIndex() <= 86080)
            {
                bool isBip01Active;
                bool isBip01Spine2Active;

                // Eğer "Bip01" aktifse, sadece "Bip01" efektini ekle
                if (isBip01Active)
                {
                    __AttachAcceShiningEffect(i, shiningTable.szShinings[i], "Bip01");
                }
                // Eğer "Bip01 Spine2" aktifse, sadece "Bip01 Spine2" efektini ekle
                else if (isBip01Spine2Active)
                {
                    __AttachAcceShiningEffect(i, shiningTable.szShinings[i], "Bip01 Spine2");
                }
            }
            ...
            {
                // Diğer durumlarda sadece "Bip01" efekti ekle
                __AttachAcceShiningEffect(i, shiningTable.szShinings[i], "Bip01");
            }
        }
    }
		...
		else if (pItem->GetSubType() == CItemData::COSTUME_AURA)
		{
...
#ifdef ENABLE_WINGS_NEW
void CInstanceBase::__AttachWingShiningEffect(int effectIndex, const char* effectFileName, const char* boneName)
{
	if (IsAffect(AFFECT_INVISIBILITY))
		return;
	if (effectIndex >= CItemData::ITEM_SHINING_MAX_COUNT)
	...
	CEffectManager::Instance().RegisterEffect(effectFileName, false, false);
	m_wingShiningEffects[effectIndex] = m_GraphicThingInstance.AttachEffectByName(0, boneName, effectFileName);
}
void CInstanceBase::__ClearWingShiningEffect(bool detaching)
{
	for (int i = 0; i < CItemData::ITEM_SHINING_MAX_COUNT; i++)
	{
		if (detaching)
			__DetachEffect(m_wingShiningEffects[i]);
		m_wingShiningEffects[i] = 0;
	}
}
#endif

#ifdef ENABLE_AURA_SYSTEM
void CInstanceBase::__AttachAuraShiningEffect(int effectIndex, const char* effectFileName, const char* boneName)
{
	if (IsAffect(AFFECT_INVISIBILITY))
	{
		return;
	...
	CEffectManager::Instance().RegisterEffect(effectFileName, false, false);
	m_auraShiningEffects[effectIndex] = m_GraphicThingInstance.AttachEffectByName(0, boneName, effectFileName);
}
#endif
...
#ifdef ENABLE_AURA_SYSTEM
void CInstanceBase::__ClearAuraShiningEffect(bool detaching)
{
	for (int i = 0; i < CItemData::ITEM_SHINING_MAX_COUNT; i++)
	{
		if (detaching)
		{
			__DetachEffect(m_auraShiningEffects[i]);
		}

		m_auraShiningEffects[i] = 0;
	}
}
#endif
...
#ifdef ENABLE_MODEL_RENDER_TARGET
bool CInstanceBase::SetWeapon(DWORD eWeapon, bool bRenderTarget)
#else
...
#ifdef ENABLE_MODEL_RENDER_TARGET
void CInstanceBase::ChangeWeapon(DWORD eWeapon, bool bRenderTarget)
#else
...
#ifdef ENABLE_MODEL_RENDER_TARGET
	if (SetWeapon(eWeapon, bRenderTarget))
#else
...
#endif
#ifdef ENABLE_WINGS_NEW
	DWORD dwWing = GetPart(CRaceData::PART_WING);
#endif
...
#ifdef ENABLE_AURA_SYSTEM
	__ClearAuraShiningEffect(false);
#endif
#ifdef ENABLE_WINGS_NEW
	__ClearWingShiningEffect(false);
#endif
#endif
	m_sAlignment = 0;
	m_byPKMode = 0;
