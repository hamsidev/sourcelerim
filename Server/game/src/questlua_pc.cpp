...
namespace quest
{
	...
	ALUA(pc_enough_inventory)
	{
		...
		if (pItem->IsSpecialStorageItem())
			bEnoughInventoryForItem = ch->GetEmptySpecialStorageSlot(pItem) != -1;
		else
	...
	ALUA(pc_change_sex)
	{
		...
		if (!ch)
			return 0;
#endif
		if (CQuestManager::instance().GetCurrentCharacterPtr()->GetWear(WEAR_COSTUME_BODY) || (CQuestManager::instance().GetCurrentCharacterPtr()->GetWear(WEAR_COSTUME_HAIR)))
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "Cinsiyet deðiþtirebilmek için kostümlerini çýkartmalýsýn.");
			lua_pushboolean(L, false);
			return 1;
		}
		return 1;
	...
	ALUA(pc_mount)
	{
		...
		if (mount_vnum)
		{
			if (ch->FindAffect(SKILL_MUYEONG))//@hinata1
				ch->RemoveAffect(SKILL_MUYEONG);//@hinata1
	...
	ALUA(pc_select_vid)
	{
		...
		{
			CQuestManager::instance().GetPC(new_ch->GetPlayerID());
			lua_pushnumber(L, ch ? (DWORD)ch->GetVID() : 0);//@hinata14
...
#ifdef ENABLE_SPECIAL_STORAGE
		LPITEM metinstone_item = ch->GetSpecialStorageItem(metinstone_cell, STONE_INVENTORY);
#else
...
teleport_area:
	...
	ALUA(pc_get_special_ride_vnum)
	{
		...
		if (pChar)
		{
			...
			if (NULL != Unique3)
			{
				...
				#ifdef ENABLE_MOUNT_COSTUME_SYSTEM
					lua_pushnumber(L, Unique3->GetValue(4)); 
				#endif
					return 3;
				}
			}
...
#ifdef ENABLE_PVP_RANKING
	int pc_rank_max_level (lua_State* L)
	{
		lua_pushnumber (L, CPvPRanking::instance().GetMaxLevel());
		return 1;
	}
	int pc_rank_min_level (lua_State* L)
	{
		lua_pushnumber (L, CPvPRanking::instance().GetMinLevel());
		return 1;
	}
	int pc_check_rank_level (lua_State* L)
	{
		LPCHARACTER pChar = CQuestManager::instance().GetCurrentCharacterPtr();

		if (NULL != pChar)
		{
			lua_pushboolean (L, CPvPRanking::instance().CheckPlayerLevel (pChar));
		}
		else
		{
			lua_pushboolean (L, false);
	...
	int pc_rank_to_base (lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		if (!ch)
		{
		...
		CPvPRanking::Instance().WarpToBase();
...
#endif
#ifdef ENABLE_PVP_RANKING
			{ "check_rank_level",		pc_check_rank_level		},
			{ "rank_max_level",		pc_rank_max_level		},
			{ "rank_min_level",		pc_rank_min_level		},
			{ "rank_to_base",		pc_rank_to_base		},
#endif
			{ NULL,			NULL			}
		};
