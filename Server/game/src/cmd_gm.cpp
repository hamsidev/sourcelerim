...
ACMD(do_item)
{
	...
	if (item)
	{
		...
		{
			int iEmptyPos;
			if ((iEmptyPos = ch->GetEmptySpecialStorageSlot(item)) == -1)
			{
				if ((iEmptyPos = ch->GetEmptyInventory(item->GetSize())) == -1)
				{
					M2_DESTROY_ITEM(item);
					ch->ChatPacket(CHAT_TYPE_INFO, "Not enough inventory space.");
					return;
...
struct FuncPurge
{
	...
	void operator () (LPENTITY ent)
	{
		...
		if (pkChr->IsNPC() && !pkChr->IsPet() && !pkChr->IsMount() && pkChr->GetRider() == nullptr)
#else
...
#ifdef ENABLE_SPECIAL_STORAGE
		ch->ChatPacket(CHAT_TYPE_INFO, " storage or st");
#endif
...
#ifdef ENABLE_SPECIAL_STORAGE
	else if (!strArg.compare(0, 2, "st") || !strArg.compare(0, 7, "storage"))
	{
		for (int i = 0; i < 5; ++i)
		{
			int j;
			...
			{
				if ((item = ch->GetItem(TItemPos(ch->GetSpecialWindowType(i), j))))
					ITEM_MANAGER::instance().RemoveItem(item, "PURGE_UPGRADE_STORAGE");
			}
		}
	}
#endif
		...
		case DoSetTypes::ALIGNMENT: // alignment
			{
				int	amount = 0;
				str_to_number(amount, arg3);
				tch->UpdateAlignment(amount - tch->GetRealAlignment());//@hinata5
			}
			break;
	}
...
ACMD(do_reload)
{
	...
	if (*arg1)
	{
		switch (LOWER(*arg1))
		{
			...
			{
				char szFileName[256];
				snprintf(szFileName, sizeof(szFileName), "%s/whell_drop_data.txt", LocaleService_GetBasePath().c_str());
				if (ITEM_MANAGER::instance().ReadWhellRewards(szFileName)) {
					ch->ChatPacket(CHAT_TYPE_INFO, "Success: %s", szFileName);
				}
				else
					ch->ChatPacket(CHAT_TYPE_INFO, "Failed: %s", szFileName);
			}
			break;
#endif
				//RELOAD_ADMIN
...
#ifdef ENABLE_PVP_RANKING
LPEVENT m_pkPvPRankingEvent = NULL;
EVENTINFO (TPvPRankingEventInfo)
{
	DWORD empty;
} ;
EVENTFUNC (pvpranking_event)
{
	TPvPRankingEventInfo* info = dynamic_cast<TPvPRankingEventInfo*> (event->info);

	if (CPvPRanking::instance().GetStatus() == 0)
	{
		return 0;
	}

	CPvPRanking::instance().Check();
...
ACMD (do_pvp_ranking)
{
	...
	if (vecArgs.size() < 5)
	{
		ch->ChatPacket (CHAT_TYPE_INFO, LC_TEXT ("Eksik bilgi girildi."));
		return;
	}

	BYTE type, minLevel, maxLevel;
	DWORD mapIndex;
	int maxKill;

	str_to_number (type, vecArgs[1].c_str());
	str_to_number (minLevel, vecArgs[2].c_str());
	str_to_number (maxLevel, vecArgs[3].c_str());
	str_to_number (maxKill, vecArgs[4].c_str());
	str_to_number (mapIndex, vecArgs[5].c_str());

	quest::CQuestManager::instance().RequestSetEventFlag ("pvp_ranking", type);

	if (type == 1)
	{
		if (m_pkPvPRankingEvent)
		{
			event_cancel (&m_pkPvPRankingEvent);
		}

		CPvPRanking::instance().Start (minLevel, maxLevel, maxKill, mapIndex);

		TPvPRankingEventInfo* info = AllocEventInfo<TPvPRankingEventInfo>();

		m_pkPvPRankingEvent = event_create (pvpranking_event, info, 3);
		BroadcastNotice (LC_TEXT ("Savas Alani etkinligi basladi.Katilmak icin Uriel ile gorusun."));
	}
	else if (type == 0)
	{
		if (m_pkPvPRankingEvent)
