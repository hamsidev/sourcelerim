...
void CInputLogin::Entergame(LPDESC d, const char * data)
{
	...
	if(quest::CQuestManager::instance().GetEventFlag("whell_event") == 1)
		ch->ChatPacket(CHAT_TYPE_COMMAND, "whellstatus %d", 1);
#endif
...
#ifdef ENABLE_RONARK_SYSTEM
	else if (CRonarkMapManager::instance().IsRonarkMap(ch->GetMapIndex()))
	{
		if (CRonarkMapManager::instance().IsRonarkActivate())
		{
			if (ch->GetGuild())
			{
				if (CRonarkMapManager::instance().GetMemberLimitCount() > CRonarkMapManager::instance().GetTotalMemberCount())
					CRonarkMapManager::instance().IncMember(ch);
				else
					ch->WarpSet(EMPIRE_START_X(ch->GetEmpire()), EMPIRE_START_Y(ch->GetEmpire()));
			}
			else
	...
	{
		CPvPRanking::instance().AddMember (ch);
	}
#endif
