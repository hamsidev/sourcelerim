...
#ifdef on
extern int64_t	CalcDamBonus(LPCHARACTER pkAttacker, LPCHARACTER pkVictim, int64_t iDam);
extern int64_t	CalcAttBonus(LPCHARACTER pkAttacker, LPCHARACTER pkVictim, int64_t iAtk);
extern int64_t	CalcBattleDamage(int64_t iDam, int iAttackerLev, int iVictimLev);
extern int64_t	CalcMeleeDamage(LPCHARACTER pAttacker, LPCHARACTER pVictim, bool bIgnoreDefense = false, bool bIgnoreTargetRating = false);
extern int64_t	CalcMagicDamage(LPCHARACTER pAttacker, LPCHARACTER pVictim);
extern int64_t	CalcArrowDamage(LPCHARACTER pkAttacker, LPCHARACTER pkVictim, LPITEM pkBow, LPITEM pkArrow, bool bIgnoreDefense = false);
#else
...
#ifdef on
extern int64_t	battle_melee_attack(LPCHARACTER ch, LPCHARACTER victim);
#else
