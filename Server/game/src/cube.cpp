...
#endif
{
	...
	if ((inven_type == INVENTORY && INVENTORY_MAX_NUM<=inven_index) || 
		...
		(inven_type == GENERAL_INVENTORY && SPECIAL_INVENTORY_MAX_NUM<=inven_index) ||
		(inven_type == BONUS_INVENTORY && SPECIAL_INVENTORY_MAX_NUM<=inven_index))
		return;

	if (inven_type != INVENTORY && inven_type != UPGRADE_INVENTORY && inven_type != BOOK_INVENTORY && inven_type != STONE_INVENTORY && inven_type != GENERAL_INVENTORY && inven_type != BONUS_INVENTORY)
		return;
#else
...
void Cube_request_result_list(LPCHARACTER ch)
{
	...
	if (resultText.length() == 0)
	{
		...
		size_t resultTextSize = resultText.size() < 20 ? 20 - resultText.size() : resultText.size() - 20;//@hinata11
		if (resultTextSize >= CHAT_MAX_LEN)//@hinata11
		{
