...
bool CGuild::RemoveMember(DWORD pid)
{
	...
	if (ch)
	{
		...
		if (ch->IsAffectFlag(AFF_RONARK_GUILD))
			ch->RemoveAffect(AFFECT_RONARK_GUILD);
#endif
	}
...
void CGuild::LoginMember(LPCHARACTER ch)
{
	...
	{
		if (!ch->IsAffectFlag(AFF_RONARK_GUILD))
			ch->AddAffect(AFFECT_RONARK_GUILD, POINT_NONE, 0, AFF_RONARK_GUILD, INFINITE_AFFECT_DURATION, 0, true);
	}
	else
	{
		if (ch->IsAffectFlag(AFF_RONARK_GUILD))
...
void CGuild::LoadGuildData(SQLMsg* pmsg)
{
	...
	#endif
#ifdef ENABLE_RONARK_SYSTEM
	str_to_number(m_data.ronark, row[13]);
#endif
...
void CGuild::RequestDisband(DWORD pid)
{
	...
	if (ch)
	{
		//Bunu ben ekledim
#ifdef ENABLE_RONARK_SYSTEM
		if (ch->RemoveAffect(AFFECT_RONARK_GUILD));
#endif
	}
...
#ifdef ENABLE_RONARK_SYSTEM
void CGuild::RequestRonarkKing(bool is_king)
{
	TPacketGDRonarkKing p;
	p.dwGuild = GetID();
	p.ronark_king = is_king;
	db_clientdesc->DBPacket(HEADER_GD_GUILD_RONARK_KING, 0, &p, sizeof(p));
}
#endif
...
#ifdef ENABLE_RONARK_SYSTEM
void CGuild::RecvRonarkKing(bool ronark_king)
{
	m_data.ronark = ronark_king;
