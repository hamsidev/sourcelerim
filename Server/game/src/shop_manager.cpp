...
LPSHOP CShopManager::CreatePCShop(LPCHARACTER ch, TShopItemTable * pTable
...
)
{
	...
	if (GOLD_MAX <= nTotalMoney)
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("20ldlÉRE AE°úÇDz© a°Ç°R» CCLö lr¨R´D´U."));
		return;
	}
	...
	if (iVal > 0)
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ÆÇ¸Å±Ý¾×ÀÇ %d %% °¡ ¼¼±ÝÀ¸·Î ³ª°¡°ÔµË´Ï´Ù"), iVal);
